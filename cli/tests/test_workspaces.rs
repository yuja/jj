...
fn test_workspaces_current_op_discarded_by_other(automatic: bool) {
    ...
    if automatic {
        ...
        │  -- operation f6295862a8f9 (2001-02-03 08:05:18) snapshot working copy
        ○  kmkuslsw hidden test.user@example.com 2001-02-03 08:05:18 8ed0355c
           (empty) RECOVERY COMMIT FROM `jj workspace update-stale`
           -- operation cbae4d96bf26 (2001-02-03 08:05:18) recovery commit
        [EOF]
        ");
    } else {
        insta::assert_snapshot!(output, @r"
        ...
        │  RECOVERY COMMIT FROM `jj workspace update-stale`
        │  -- operation 0d1ba46a69ba (2001-02-03 08:05:18) snapshot working copy
