...
fn test_default_config() {
    ...
    let is_maskable_char = |c: u8| c.is_ascii_alphanumeric() | matches!(c, b'-' | b'.' | b'_');
    let is_maskable_str = |s: &str| s.as_bytes().iter().all(|&c| is_maskable_char(c));
    let maskable_op_user = whoami::fallible::hostname().is_ok_and(|s| is_maskable_str(&s))
        && whoami::fallible::username().is_ok_and(|s| is_maskable_str(&s));
